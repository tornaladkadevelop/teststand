...
class TestUBTZ:
    ...
    def st_test_20(self) -> bool:
        ...
        for i in self.list_ust_bmz_volt:
            ...
            self.fault.debug_msg(f'{in_a1 = } (T), {in_a2 = } (F), {in_a5 = } (F), {in_a6 = } (T)', 'purple')
            if in_a1 is True and in_a5 is False and in_a2 is False and in_a6 is True:
                self.fault.debug_msg('тест 2 положение выходов соответствует', 'green')
                ...
                self.fault.debug_msg('тест 2 положение выходов не соответствует', 'red')
                if in_a1 is False:
    ...
    def st_test_30(self) -> bool:
        ...
        for n in self.list_ust_tzp_volt:
            ...
            while in_a6 is True and sub_timer <= 370:
                ...
                self.fault.debug_msg(f'времени прошло: {sub_timer}', 'orange')
        ...
        self.fault.debug_msg(f"ТЗП дельта t: {self.list_delta_t_tzp}", 'blue')
        self.mysql_conn.mysql_ins_result("исправен", '1')
        return True
    ...
    def subtest_32(self, i, k):
        ...
        self.mysql_conn.mysql_add_message(f'уставка {self.list_ust_bmz_num[k]}: '
                                          f'дельта t: {self.calc_delta_t_bmz:.1f}')
        self.fault.debug_msg(f'{in_a1 = }, {in_a2 = }, {in_a5 = }, {in_a6 = }', 'purple')
        if in_a1 is True and in_a5 is False and in_a2 is False and in_a6 is True:
            pass
        else:
    ...
    def result_test_ubtz(self):
        for g1 in range(len(self.list_delta_t_bmz)):
            ...
            self.fault.debug_msg(f"запись уставок МТЗ в БД: {self.list_ust_bmz_num[g1]} "
        ...
        for g2 in range(len(self.list_delta_t_tzp)):
            ...
            self.fault.debug_msg(f"запись уставок ТЗП в БД: {self.list_ust_tzp_num[g2]} "
